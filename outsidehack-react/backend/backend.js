...
function queryStateHandler(req) {
    // Verify all requests.
...
function changeStateHandler(req) {
    // Verify all requests.
    const payload = verifyAndDecode(req.headers.authorization);
    const { channel_id: channelId, opaque_user_id: opaqueUserId } = payload;
    ...
    //just have to return something to client
    return 0;
}
...
function sendStateBroadcast(channelId) {
    ...
    // const apiHost = 'localhost.rig.twitch.tv:3000';
    request(
...
(async () => {
  ...
  server.route({
      method: 'GET',
      path: '/samptwitch/query',
      handler: queryStateHandler,
  });

  server.route({
      method: 'POST',
      path: '/samptwitch/change',
